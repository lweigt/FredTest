...
# ─────────────────────────────────────────────
# ANTHROPIC FEHLERBEHANDLUNG (zentral)
# ─────────────────────────────────────────────

def anthropic_fehler(e: Exception) -> HTTPException:
    """Wandelt jeden Anthropic-SDK-Fehler in eine sprechende HTTP-Antwort um."""
    if isinstance(e, anthropic.AuthenticationError):
        return HTTPException(status_code=401, detail={
            "fehler": "Ungültiger API-Key",
            "meldung": (
                "Der angegebene Anthropic API-Key ist ungültig oder abgelaufen. "
                "Bitte prüfe den Key unter https://console.anthropic.com und setze ihn neu: "
                "export ANTHROPIC_API_KEY='sk-ant-...'"
            ),
            "code": "INVALID_API_KEY",
            "hinweis": "Ohne gesetzten API-Key läuft das System vollständig regelbasiert weiter."
        })
    if isinstance(e, anthropic.PermissionDeniedError):
        return HTTPException(status_code=403, detail={
            "fehler": "Fehlende API-Berechtigung",
            "meldung": (
                "Der API-Key hat keine Berechtigung für das Modell 'claude-sonnet-4-20250514'. "
                "Bitte prüfe dein Anthropic-Konto unter https://console.anthropic.com."
            ),
    ...
    if isinstance(e, anthropic.RateLimitError):
        return HTTPException(status_code=429, detail={
            ...
            "meldung": "Zu viele Anfragen an die Claude API. Bitte warte kurz und versuche es erneut.",
            "code": "RATE_LIMIT"
        })
    if isinstance(e, anthropic.APIConnectionError):
        return HTTPException(status_code=503, detail={
            "fehler": "Claude API nicht erreichbar",
            "meldung": (
    ...
    if isinstance(e, anthropic.BadRequestError):
        return HTTPException(status_code=400, detail={
            "fehler": "Ungültige Anfrage an Claude API",
            "meldung": str(e),
            "code": "BAD_REQUEST"
        })
    # Fallback
...
async def modul_xai(inp: ObjektInput, basis: ModulErgebnis, human: HumanAnalogueErgebnis) -> XAIErgebnis:
    ...
    if not ANTHROPIC_API_KEY:
        ...
        return XAIErgebnis(treiber=treiber, begruendung=begruendung,
                           methodik="Regelbasierte Faktorzerlegung (Hedonic Pricing)")

    # Claude-Aufruf – Anthropic-Fehler werden nach oben weitergegeben
...
Schreibe sachlich, präzise, ohne Überschrift."""
    ...
    # Anthropic-Fehler werden nach oben weitergegeben und im Endpoint behandelt
